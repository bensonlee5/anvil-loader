...
REMOTE_NAME="${REMOTE_NAME:-Jetson Thor}"
...
if ! is_reachable; then
  printf '%s\n' "${REMOTE_NAME} is already unreachable at ${REMOTE_HOST}."
  exit 0
fi

printf '%s\n' "Requesting ${REMOTE_NAME} shutdown at ${ssh_target}..."
...
while is_reachable; do
  if (( SECONDS >= deadline )); then
    printf '%s\n' "Timed out waiting for ${REMOTE_NAME} to stop responding at ${REMOTE_HOST}." >&2
    exit 1
  fi
  sleep 2
done

printf '%s\n' "${REMOTE_NAME} is offline."
